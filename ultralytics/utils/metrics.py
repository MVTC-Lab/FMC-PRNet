...
#
#     【修改说明】:
#     此处已将 CIoU 分支替换为 WIoU v3 (Wise-IoU v3)。
#     当传入 CIoU=True 时，实际上计算的是 WIoU v3 Loss。
#     返回值为 (1 - WIoU_Loss)，以适配外部 loss = 1 - result 的计算逻辑。
#     """
#
#     # Get the coordinates of bounding boxes
#     if xywh:  # transform from xywh to xyxy
...
#
#             # --- WIoU v1 部分 ---
#             # Distance Attention: R_WIoU = exp(rho2 / c2)
#             dist_cost = torch.exp(rho2 / c2)
#             loss_iou = 1.0 - iou
#             loss_wiou_v1 = dist_cost * loss_iou
#
#             # --- WIoU v3 部分 (动态非单调聚焦) ---
#             # 分离梯度，计算离群度 beta
#             loss_detach = loss_iou.detach()
#             beta = loss_detach / (loss_detach.mean() + eps)
#
#             # 计算聚焦系数 r
#             # 这里使用 WIoU v3 推荐的稳定实现 (alpha=1.9, delta=3.0)
#             # r = exp(-((beta * 1.9) ** 2)) 这里的逻辑可以根据论文微调
#             # 为了防止梯度过大，通常加上 clamp 或者使用高斯衰减
#             alpha = 1.9
#             r = torch.exp(-((beta * alpha) ** 2))
#
#             # 最终的 WIoU Loss
#             loss_wiou_v3 = loss_wiou_v1 * r
#
#             # 【关键】因为外部 loss.py 调用的是 (1.0 - result)，
#             # 所以我们要返回 (1.0 - loss_wiou_v3)，这样外部计算结果就是 loss_wiou_v3
#             return 1.0 - loss_wiou_v3
#
#         # 2. 如果是 DIoU (保留原逻辑)
#         if DIoU:
#             c2 = cw.pow(2) + ch.pow(2) + eps
...
#     """
#     计算 NWD (Normalized Wasserstein Distance) Loss
#     Args:
#         pred: 预测框 [x1, y1, x2, y2]
#         target: 真实框 [x1, y1, x2, y2]
#         constant: NWD 的常数 C，通常取 12.5
#     Returns:
#         nwd: NWD 指标 (0~1，越大越好)
#     """
#     # 1. 将 xyxy 转换为 center_x, center_y, width, height
...
#
#     # 2. 计算 Wasserstein Distance (W2)
#     # 对于轴对齐的高斯分布 (Bounding Box)，公式简化为中心点距离 + 宽高差异
#     b1_cx, b1_cy, b1_w, b1_h = cx1, cy1, w1, h1
#     b2_cx, b2_cy, b2_w, b2_h = cx2, cy2, w2, h2
#
#     # 距离平方
#     center_distance_sq = (b1_cx - b2_cx) ** 2 + (b1_cy - b2_cy) ** 2
#
#     # 宽高差异 (根据 NWD 论文公式)
#     wh_distance_sq = ((b1_w - b2_w) / 2) ** 2 + ((b1_h - b2_h) / 2) ** 2
#
#     wasserstein_2 = center_distance_sq + wh_distance_sq
#
#     # 3. 计算 Normalized Wasserstein Distance (NWD)
#     # constant (C) 是一个与数据集相关的常数，通常取 12.5 左右
