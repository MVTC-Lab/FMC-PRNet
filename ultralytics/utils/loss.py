...
#             stride: torch.Tensor,
#     ) -> tuple[torch.Tensor, torch.Tensor]:
#         """Compute IoU and DFL losses for bounding boxes with NWD integration."""
#
#         # 计算权重
#         weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
#
#         # ==========================================
#         # 1. 计算原始 IoU Loss (CIoU)
#         # ==========================================
#         iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
#         loss_iou_original = ((1.0 - iou) * weight).sum() / target_scores_sum
#
#         # ==========================================
#         # 2. 计算 NWD Loss (新增部分)
#         # ==========================================
#         # 调用 wasserstein_loss 计算 NWD 指标 (0~1)
#         nwd = wasserstein_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask])
#
#         # 计算 NWD 损失 (1 - nwd)，并应用相同的权重归一化
...
#         # ==========================================
#         # nwd_ratio: NWD 的权重。
#         # 对于小目标检测，建议 0.5 ~ 0.7。如果主要为了防震锤，0.5 是一个稳健的起点。
#         nwd_ratio = 0.5
#         loss_box = loss_iou_original * (1 - nwd_ratio) + loss_nwd * nwd_ratio
#
#         # ==========================================
#         # 4. DFL Loss (保持不变)
#         # ==========================================
